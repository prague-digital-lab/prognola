...
<template>
  <div class="relative">
    <p class="text-xs px-1 py-1 rounded text-gray-800 hover:bg-gray-100 mb-7" @click="expanded ? close() : expand()">
      ...
      <span class="">{{ selected_income_category ? selected_income_category.department.name + ':' : '' }}</span>

      {{ selected_income_category ? selected_income_category.name : 'zvolit kategorii' }}
    </p>
      ...
      <div v-if="expanded"
        ...
        <div class="max-h-[50vh] overflow-auto">
          <div v-for="department in filtered_categories_grouped_by_department">
            <p class="px-2 py-1 text-gray-500 text-xs">
              ...
              {{department[0].department.name}}</p>

          <p class="px-2 py-1 hover:bg-gray-100 text-gray-700 text-sm" v-for="category in department"
             @click="selectCategory(category)">
            {{ category.name }}</p>
          </div>

          <p class="px-2 py-1 text-gray-500 text-sm" v-if="filtered_categories.length === 0">
            žádná kategorie</p>
        </div>
      </div>
    </Transition>
